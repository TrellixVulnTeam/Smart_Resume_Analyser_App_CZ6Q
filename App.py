...
connection = pymysql.connect(host='	sql101.epizy.com',user='epiz_31992575',password='wuKeSF1zqiVgbr',db='epiz_31992575_resumeanalyzer')
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'admin' and ad_password == 'admin':
                ...
                df = pd.DataFrame(list(data), columns=['ID', 'Name', 'Email', 'Resume Score', 'Timestamp', 'Total Page','Predicted Field', 'User Level', 'Actual Skills', 'Recommended Skills','Recommended Course'])
                st.dataframe(df)
